...
if(isset($_POST['Amount']) && !empty($_POST['Amount']) && isset($_POST['Acc_no1']) && !empty($_POST['Acc_no1']) ){
	...
	if($amount>0){
		...
		if(mysqli_num_rows($query3_data)==1){
			...
			$a=$query3_row['Amount'];
			if($amount<=$a){
			$query7="SELECT COUNT(*) as Trans_count FROM transaction";
$query7_data=mysqli_query($con,$query7);
$query7_row=mysqli_fetch_assoc($query7_data);
		$Trans_id=$query7_row['Trans_count']+1;
    $remark='Check_transfer';
    $query1="INSERT INTO transaction (Trans_id,Date,Acc_no1,Acc_no2,Remark,Amount,Emp_id) VALUES('$Trans_id','$date','$acc_no','$acc_no_debit','$remark','$amount','$Emp_id')";
