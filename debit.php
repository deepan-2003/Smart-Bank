...
$query4="SELECT COUNT(*) as Trans_count FROM transaction";
$query4_data=mysqli_query($con, $query4);
$query4_row=mysqli_fetch_assoc($query4_data);
if ($query4_row !== null) {
  $Trans_id = $query4_row['Trans_count'] + 1;
} else {
  // Handle the case when $query4_row is null
  // Assign a default value or perform alternative actions
  
  $Trans_id = 1;
  echo $Trans_id; // Default value if 'Trans_count' is not available
}
